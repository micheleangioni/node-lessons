...
- The endpont should be able to receive a `get random job` with a `location` data and answer with a random job from the free Gihub Jobs API
- Once connected to the Web Socket endpoint, every 3 seconds it should automatically end a random jobs, taken from the same API and without location filter, to the connected client
